...
export default function Notification() {
    ...
    const fetchData = async () => {
        setLoading(true)
        try {
            const response = await NotificationService.getAllNotificationOfUser(tokenData.email)
    ...
    const filteredNoti = searchQuery ? notis.filter((noti) => {
        return (
            noti.description.toLowerCase().includes(searchQuery.toLowerCase())
        )
    }) : notis
                            ...
                            <div className="card w-100 shadow-sm">
                                <div className="card-body p-4">
                                    <div className='d-flex justify-content-end align-items-center mb-4'>
                                        <button onClick={generatePdf} className='btn btn-outline-dark mx-2'>Export PDF</button>
                                    </div>
                                    <div className="d-flex justify-content-between align-items-center mb-2">

                                        <h5 className="card-title fw-semibold">List Of Notifications</h5>
                                        <form className="position-relative">
                                            <input
                                                value={searchQuery}
                                                onChange={(e) => setSearchQuery(e.target.value)}
                                                type="text" className="form-control search-chat py-2 ps-5" id="text-srh" placeholder="Search Description" />
                                                                ...
                                                                return (
                                                                    <tr key={noti._id}>
                                                                        <td className="border-bottom-0" >
                                                                            <p className="fw-normal mb-1">{noti.description}</p>
                                                                        </td>
                                                                        <td className="border-bottom-0" >
                                                                            <p className="fw-normal mb-1">{noti.from}</p>
                                                                        </td>
                                                                        <td className="border-bottom-0" >
                                                                            <p className="mb-0 fw-normal">{noti.toEmail}</p>
                                                                        </td>
                                                                        <td className="border-bottom-0" >
                                                                            <p className="mb-0 fw-normal">{DateFormatter.formatDate(noti.created_at)}</p>
                                                                        </td>
                                                                        {
                                                                            noti.seen ? (
                                                                                <td className="border-bottom-0">
